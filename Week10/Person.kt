...
// print out the number of persons whose first name has less than 5 letters, and who live in a city
// with an odd zip code.
// need to change the method that is called when the person is created in the persons method
// from calling the method name we change it to calling the method firstName 
// yield ( Person(firstName(), phone, AddressGenerator.address(200)) )
        
fun main () {
    PersonGenerator.persons(10000).
            filter {it.name.length < 5 }
            .filter {it.address.zip.toInt()  % 2 !=0 }
            .forEach{println(it)}
}

fun main () {
val seq: Sequence<Person> = sequence {(PersonGenerator.persons(10000))}
    //  PersonGenerator.persons(10000).
            //    var group = seq.groupBy{it.name.length}.asSequence()..forEach{println(it.component1.toString()values.size())}
